                                   ...
                                   int crm, int opc1, int opc2)
{
    ...
    if (!(r->type & ARM_CP_OVERRIDE)) {
        ...
        if (oldreg && !(oldreg->type & ARM_CP_OVERRIDE)) {
            LOGE("Register redefined: cp=%d %d bit "
                                       ...
                                       const ARMCPRegInfo *r, void *opaque)
{
    ...
    EMASSERT(!((r->type & ARM_CP_64BIT) && (r->opc2 || r->crn)));
    /* op0 only exists in the AArch64 encodings */
    EMASSERT((r->state != ARM_CP_STATE_AA32) || (r->opc0 == 0));
    /* AArch64 regs are all 64 bit so ARM_CP_64BIT is meaningless */
    EMASSERT((r->state != ARM_CP_STATE_AA64) || !(r->type & ARM_CP_64BIT));
    ...
    if (r->state != ARM_CP_STATE_AA32) {
        ...
        /* EMASSERT our permissions are not too lax (stricter is fine) */
        EMASSERT((r->access & ~mask) == 0);
    }
    ...
    if (!(r->type & (ARM_CP_SPECIAL|ARM_CP_CONST))) {
        if (r->access & PL3_R) {
            EMASSERT(r->fieldoffset || r->readfn);
        }
        if (r->access & PL3_W) {
            EMASSERT(r->fieldoffset || r->writefn);
        }
    }
    /* Bad type field probably means missing sentinel at end of reg list */
    EMASSERT(cptype_valid(r->type));
